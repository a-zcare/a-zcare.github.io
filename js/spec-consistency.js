(() => {
  const correct = {
    ...
    Protection: [
      ...
      'Secure Boot · privacy-first permissions',
    ],
  };

  function fix() {
    const detail = document.querySelector('#phoneView .spec-detail');
    if (!detail) return;

    const key = detail.querySelector('small')?.textContent?.trim();
    const data = correct[key];
    if (!data) return;
    ...
    if (paragraphs[0]) paragraphs[0].textContent = data[1];
    if (paragraphs[1]) paragraphs[1].textContent = data[2];
  }

  document.addEventListener('click', (event) => {
    if (event.target.closest('[data-open="specs"], [data-spec], [data-back]')) {
      setTimeout(fix, 0);
    }
  });
